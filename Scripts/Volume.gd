...
func _ready() -> void:
	...
	AudioServer.set_bus_volume_db(MASTER, linear2db(master_vol))
	AudioServer.set_bus_volume_db(SFX, linear2db(sfx_vol))
	AudioServer.set_bus_volume_db(MUSIC, linear2db(music_vol))
...
func _on_SfxSlider_value_changed(value: float) -> void:
	...
	AudioServer.set_bus_volume_db(SFX, linear2db(value))
